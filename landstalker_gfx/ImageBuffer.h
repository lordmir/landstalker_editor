...
public:
	...
	void InsertTile(std::size_t x, std::size_t y, uint8_t palette_index, const Tile& tile, const Tileset& tileset);
	bool WritePNG(const std::string& filename, const std::vector<Palette>& pals);
	void InsertBlock(std::size_t x, std::size_t y, uint8_t palette_index, const BigTile& block, const Tileset& tileset);
