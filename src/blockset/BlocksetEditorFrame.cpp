...
void BlocksetEditorFrame::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
...
void BlocksetEditorFrame::SetDrawTile(const Landstalker::Tile& tile)
{
...
void BlocksetEditorFrame::ExportBin(const std::string& filename) const
{
	...
	auto sz = Landstalker::BlocksetCmp::Encode(*m_blocks->GetData(), bytes.data(), 65536);
	bytes.resize(sz);
...
void BlocksetEditorFrame::ExportPng(const std::string& filename) const
{
	...
	int blockwidth = Landstalker::MapBlock::GetBlockWidth() * m_tiles->GetData()->GetTileWidth();
	int blockheight = Landstalker::MapBlock::GetBlockHeight() * m_tiles->GetData()->GetTileHeight();
...
void BlocksetEditorFrame::ExportCsv(const std::string& filename) const
{
	...
	for (std::size_t y = 0; y < m_blocks->GetData()->size(); ++y)
	{
		...
		{
			fs << Landstalker::StrPrintf("0x%04X", m_blocks->GetData()->at(y).GetTile(x).GetTileValue());
			if (x == 3)
			{
				fs << std::endl;
			}
...
{
	Landstalker::ByteVector bytes = Landstalker::ReadBytes(filename);
	m_blocks->GetData()->clear();
	Landstalker::BlocksetCmp::Decode(bytes.data(), bytes.size(), *m_blocks->GetData());
	m_editor->RedrawTiles();
}
...
void BlocksetEditorFrame::ImportCsv(const std::string& filename)
{
	...
	std::vector<Landstalker::Tile> tiles(4);
	std::string row;
	std::string cell;
	uint32_t val;
	while (std::getline(fs, row))
	{
		...
		{
			std::getline(ss, cell, ',');
			Landstalker::StrToInt(cell, val);
			tiles[x] = val;
		}
		blocks.push_back(Landstalker::MapBlock(tiles.cbegin(), tiles.cend()));
	}
...
void BlocksetEditorFrame::RefreshProperties(wxPropertyGridManager& props) const
{
	...
	{
		InitPaletteList();
		props.GetGrid()->SetPropertyValue("Name", wxString(m_blocks->GetName()));
		props.GetGrid()->SetPropertyValue("Tileset", wxString(m_gd->GetRoomData()->GetTileset(m_blocks->GetTileset())->GetName()));
		props.GetGrid()->SetPropertyValue("Primary Blockset", wxString(m_gd->GetRoomData()->GetBlockset(m_blocks->GetTileset(), m_blocks->GetPrimary(), 0)->GetName()));
...
void BlocksetEditorFrame::ProcessEvent(int id)
{
	...
	case ID_EDIT_CUT:
		if (m_editor->GetMode() == BlocksetEditorCtrl::Mode::BLOCK_SELECT && m_editor->IsBlockSelectionValid())
		{
			...
			m_editor->SetSelectedBlock(Landstalker::MapBlock());
		}
		else if (t != -1 && b != -1)
		{
			m_tileclipboard = m_editor->GetTile(b, t);
			m_editor->SetTile(b, t, Landstalker::Tile());
		}
		break;
	case ID_COPY:
	case ID_EDIT_COPY:
