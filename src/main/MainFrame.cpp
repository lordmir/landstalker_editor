...
void MainFrame::OpenRomFile(const wxString& path)
{
    try
    {
        ...
        m_g = std::make_shared<Landstalker::GameData>();
        {
...
MainFrame::ReturnCode MainFrame::SaveLabelsFile(std::string path)
{
    ...
    Landstalker::Labels::SaveData(path_obj.string());
...
void MainFrame::OpenLabelsFile(std::string path)
{
    ...
    Landstalker::Labels::LoadData(path_obj.string());
}
...
void MainFrame::RefreshEditor()
{
    ...
    case Mode::STRING:
        // Display Strings
        GetStringEditor()->SetMode(static_cast<Landstalker::StringData::Type>(m_seldata));
