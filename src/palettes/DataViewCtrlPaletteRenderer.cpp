...
bool DataViewCtrlPaletteRenderer::SetValue(const wxVariant& value)
{
    ...
    {
        m_pal = static_cast<Landstalker::PaletteEntry*>(value.GetAny().As<void*>());
    }
    return true;
}
...
Landstalker::Palette::Colour DataViewCtrlPaletteRenderer::GetCursorColour()
{
    int pos = GetCursorPosition();
    if (m_pal && pos != -1)
    {
    ...
    return Landstalker::Palette::Colour();
}
