...
void PaletteEditor::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
	m_gd = gd;
	if (m_gd == nullptr)
	{
...
void PaletteEditor::SelectPalette(std::shared_ptr<Landstalker::Palette> pal)
{
	m_selected_palette_name = "";
	m_selected_palette_entry = nullptr;
...
void PaletteEditor::OnDoubleClick(wxMouseEvent& evt)
{
	...
	if ((colour_selected != -1) && (!m_locked[colour_selected]))
	{
		...
		if (dlg.ShowModal() == wxID_OK)
		{
			...
			auto result = Landstalker::Palette::Colour::CreateFromBGRA(colour.GetRGBA());
...
{
	if (index < 0 || !m_selected_palette) return *wxGREY_BRUSH;
