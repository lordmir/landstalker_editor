...
void CharacterDialog::AddRow()
{
    ...
    m_ctrl_list->Select(wxDataViewItem(reinterpret_cast<void*>(static_cast<intptr_t>(m_model->GetRowCount()))));
}
...
void CharacterDialog::DeleteRow()
{
    if (m_ctrl_list->HasSelection())
    {
        ...
        {
            m_ctrl_list->Select(wxDataViewItem(reinterpret_cast<void*>(static_cast<intptr_t>(sel - 1))));
        }
