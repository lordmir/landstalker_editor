...
void EntityControlFrame::SetEntities(const std::vector<Landstalker::Entity>& entities)
{
    m_entities = entities;
    UpdateUI();
}
...
{
    return Landstalker::StrWPrintf(L"%02d: (%04.1f, %04.1f, %04.1f) %ls", index + 1, m_entities[index].GetXDbl(),
