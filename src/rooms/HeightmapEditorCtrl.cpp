...
void HeightmapEditorCtrl::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
    m_g = gd;
    Refresh(true);
}
...
void HeightmapEditorCtrl::UpdateWarps(const std::vector<Landstalker::WarpList::Warp>& warps)
{
    if (m_g)
    {
        m_warps = warps;
...
void HeightmapEditorCtrl::UpdateEntities(const std::vector<Landstalker::Entity>& entities)
{
    if (m_g)
    {
        m_entities = entities;
...
const std::vector<Landstalker::TileSwap>& HeightmapEditorCtrl::GetTileswaps() const
{
    return m_swaps;
}
...
const std::vector<Landstalker::Door>& HeightmapEditorCtrl::GetDoors() const
{
    return m_doors;
}
...
void HeightmapEditorCtrl::RefreshStatusbar()
{
    ...
    if (m_hovered.first != -1)
    {
        ...
        msg = Landstalker::StrPrintf("(%04d, %04d) : Z:%02d R:%01X T:%02X", m_hovered.first, m_hovered.second, z, r, t);
    }
    ...
    if (m_map)
    {
        ...
        for (int iy = 0; iy < m_map->GetHeightmapHeight(); ++iy)
        {
            for (int ix = 0; ix < m_map->GetHeightmapWidth(); ++ix)
            {
                ...
                {
                    std::string lbl1 = Landstalker::StrPrintf("%02X", type);
                    std::string lbl2 = Landstalker::StrPrintf("%X", z);
                    std::string lbl3 = Landstalker::StrPrintf("%X", restrictions);
                    wxCoord tw, th;
...
void HeightmapEditorCtrl::DrawDoors(wxDC& dc)
{
    ...
    for (int i = static_cast<int>(m_doors.size()) - 1; i >= 0; --i)
    {
        ...
        if (m_map && GetHeightmapCellType(door.x, door.y) == static_cast<int>(Landstalker::Tilemap3D::FloorType::DOOR_NW))
        {
            DrawCellRange(dc, door.x, door.y, 1, Landstalker::Door::SIZES.at(door.size).first, 2);
        }
        else
        {
            DrawCellRange(dc, door.x, door.y, Landstalker::Door::SIZES.at(door.size).first, 1, 2);
        }
    }
}
...
void HeightmapEditorCtrl::DrawTileSwaps(wxDC& dc)
{
    ...
    for (int i = static_cast<int>(m_swaps.size()) - 1; i >= 0; --i)
    {
        ...
        const Landstalker::TileSwap& swap = m_swaps.at(i);
...
int HeightmapEditorCtrl::GetFirstDoorRegion(const Coord& c)
{
    ...
    {
        const Landstalker::Door& door = m_doors.at(i);
...
{
    Landstalker::TileSwap* swap = nullptr;
    if (m_preview_swap && IsSwapSelected())
    {
    ...
    {
        Landstalker::HMPoint2D point = { x - swap->heightmap.dst_x + swap->heightmap.src_x,
