...
Map3DEditor::Map3DEditor(wxWindow* parent, RoomViewerFrame* frame, Landstalker::Tilemap3D::Layer layer)
    : wxScrolledCanvas(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE | wxWANTS_CHARS),
      m_g(nullptr),
      m_map(nullptr),
      m_map_disp(nullptr),
      m_layer_buf(std::make_unique<ImageBufferWx>()),
      m_bg_buf(std::make_unique<ImageBufferWx>()),
      m_tileset(nullptr),
      m_pal(nullptr),
      m_blockset(nullptr),
      m_frame(frame),
...
void Map3DEditor::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
    m_g = gd;
    Refresh(true);
}
...
void Map3DEditor::SetRoomNum(uint16_t roomnum)
{
    if (m_g)
    {
        ...
        m_map_disp = std::make_shared<Landstalker::Tilemap3D>(*m_map);
...
const std::vector<Landstalker::TileSwap>& Map3DEditor::GetTileswaps() const
{
    return m_swaps;
}
...
{
    m_swaps.push_back(Landstalker::TileSwap());
...
const std::vector<Landstalker::Door>& Map3DEditor::GetDoors() const
{
    return m_doors;
}
...
{
    m_doors.push_back(Landstalker::Door());
...
#define DECDSZ(X, MAX) X = static_cast<decltype(X)>((static_cast<int>(X) + MAX - 1) % MAX)
    ...
    case 'A':
        if (IsDoorSelected())
        {
            ...
            {
                DECDSZ(m_doors[GetSelectedDoor() - 1].size, Landstalker::Door::SIZES.size());
                upd = true;
            }
    ...
    case 'D':
        if (IsDoorSelected())
        {
            if (modifiers == (wxMOD_CONTROL | wxMOD_SHIFT))
            {
                INCDSZ(m_doors[GetSelectedDoor() - 1].size, Landstalker::Door::SIZES.size());
                upd = true;
            }
            else if (modifiers == wxMOD_CONTROL)
            {
...
void Map3DEditor::RefreshStatusbar()
{
    ...
    if (m_hovered.first != -1)
    {
        ...
        hovmsg = Landstalker::StrPrintf("Cursor (%04d, %04d) : Block %d", m_hovered.first, m_hovered.second, i);
    }
    if (IsBlockSelected())
    {
        selmsg = Landstalker::StrPrintf("Selected Block %d", m_selected_block);
    }
    if (IsSwapSelected() || IsDoorSelected())
    {
        int sel = m_selected_region & 0xFF;
        swpmsg = Landstalker::StrPrintf("Selected %s #%d", IsSwapSelected() ? "tile swap" : "door", sel);
        if (m_preview_swap)
        {
...
void Map3DEditor::DrawMap(wxDC& dc)
{
    ...
    if (m_map_disp)
    {
        ...
        for (int y = 0; y < m_map_disp->GetHeight(); ++y)
        {
            for (int x = 0; x < m_map_disp->GetWidth(); ++x)
            {
                int xp = (m_map_disp->GetHeight() - 1 + x - y) * TILE_WIDTH + (m_layer == Landstalker::Tilemap3D::Layer::BG ? TILE_WIDTH : 0);
                ...
                {
                    bool pri = false;
                    for (std::size_t i = 0; i < Landstalker::MapBlock::GetBlockSize(); ++i)
                    {
                        pri = pri || m_blockset->at(blk).GetTile(i).Attributes().getAttribute(Landstalker::TileAttributes::Attribute::ATTR_PRIORITY);
                    }
                    if (pri)
                    {
                ...
                {
                    wxString t = Landstalker::StrPrintf("%03X", blk);
...
void Map3DEditor::DrawTiles()
{
    if (m_redraw)
    {
        ...
        if (m_layer == Landstalker::Tilemap3D::Layer::FG)
        {
            m_bg_buf->Clear();
            m_bg_buf->Insert3DMapLayer(0, 0, 0, Landstalker::Tilemap3D::Layer::BG, m_map_disp, m_tileset, m_blockset, false);
        }
...
void Map3DEditor::DrawTileSwaps(wxDC& dc)
{
    ...
    for (const auto& s : m_swaps)
    {
        ...
        auto lines = s.GetMapRegionPoly(Landstalker::TileSwap::Region::UNDEFINED, TILE_WIDTH, TILE_HEIGHT);
        std::vector<wxPoint> wxpoints = ToWxPoints(lines);
        auto sp = GetScreenPosition(s.GetTileOffset(Landstalker::TileSwap::Region::SOURCE, m_map, m_layer));
        auto dp = GetScreenPosition(s.GetTileOffset(Landstalker::TileSwap::Region::DESTINATION, m_map, m_layer));
        auto [hx, hy] = GetScreenPosition(m_hovered);
        m_swap_regions.emplace_back(
                ToWxPoints(Landstalker::TileSwap::OffsetRegionPoly(lines, sp)),
                ToWxPoints(Landstalker::TileSwap::OffsetRegionPoly(lines, dp)));
        bool hovered = false;
        if (!m_dragging)
        {
...
void Map3DEditor::DrawDoors(wxDC& dc)
{
    ...
    for (const auto& d : m_doors)
    {
        ...
        {
            auto [valid, lines] = d.GetMapRegionPoly(m_map_disp, TILE_WIDTH, TILE_HEIGHT);
            auto wxpoints = ToWxPoints(lines);
            auto pos = GetScreenPosition(d.GetTileOffset(m_map_disp, m_layer));
            auto [hx, hy] = GetScreenPosition(m_hovered);
            m_door_regions.push_back(ToWxPoints(Landstalker::Door::OffsetRegionPoly(lines, pos)));
            wxColor colour = valid ? wxColour(255, 0, 255) : wxColour(255, 0, 255);
            if (Pnpoly(m_door_regions.back(), hx, hy))
            {
                colour = colour.ChangeLightness(150);
            }
            dc.SetPen(wxPen(colour, di == m_selected_region ? 3 : 1, wxPENSTYLE_SHORT_DASH));
...
Map3DEditor::Coord Map3DEditor::GetCellPosition(int screenx, int screeny) const
{
    ...
    int ix = 0, iy = 0;
    int offsetx = (m_map->GetHeight() - 1) * TILE_WIDTH + (m_layer == Landstalker::Tilemap3D::Layer::BG ? TILE_WIDTH : 0);
...
{
    int xp = (m_map->GetHeight() - 1 + pos.first - pos.second) * TILE_WIDTH + (m_layer == Landstalker::Tilemap3D::Layer::BG ? TILE_WIDTH : 0);
...
void Map3DEditor::GeneratePreview()
{
    if (IsSwapSelected())
    {
        *m_map_disp = *m_map;
        if (m_layer == Landstalker::Tilemap3D::Layer::FG)
        {
            m_swaps.at(GetSelectedSwap() - 1).DrawSwap(*m_map_disp, Landstalker::Tilemap3D::Layer::FG);
        }
        m_swaps.at(GetSelectedSwap() - 1).DrawSwap(*m_map_disp, Landstalker::Tilemap3D::Layer::BG);
    }
    else if (IsDoorSelected())
    {
        *m_map_disp = *m_map;
        if (m_layer == Landstalker::Tilemap3D::Layer::FG)
        {
            m_doors.at(GetSelectedDoor() - 1).DrawDoor(*m_map_disp, Landstalker::Tilemap3D::Layer::FG);
        }
    }
    else
    {
