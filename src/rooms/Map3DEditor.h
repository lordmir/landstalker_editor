...
public:
	...
	Map3DEditor(wxWindow* parent, RoomViewerFrame* frame, Landstalker::Tilemap3D::Layer layer);
	virtual ~Map3DEditor();
	...
	const std::vector<Landstalker::TileSwap>& GetTileswaps() const;
	int GetTotalTileswaps() const;
	void AddTileswap();
	void DeleteTileswap();

	const std::vector<Landstalker::Door>& GetDoors() const;
...
private:
	...
	std::shared_ptr<Landstalker::Tilemap3D> m_map;
	mutable std::shared_ptr<Landstalker::Tilemap3D> m_map_disp;
	std::unique_ptr<ImageBufferWx> m_layer_buf;
	std::unique_ptr<ImageBufferWx> m_bg_buf;

	std::shared_ptr<Landstalker::Tileset> m_tileset;
	std::shared_ptr<Landstalker::Palette> m_pal;
	std::shared_ptr<Landstalker::Blockset> m_blockset;

	RoomViewerFrame* m_frame;
	uint16_t m_roomnum;
	Landstalker::Tilemap3D::Layer m_layer;
	int m_width;
	int m_height;
	bool m_redraw;
	bool m_repaint;
	...
	std::vector<Landstalker::Door> m_preview_doors;
	std::vector<Landstalker::TileSwap> m_preview_swaps;
