...
public:
	...
	const Landstalker::Entity& GetSelectedEntity() const;
	int GetSelectedEntityIndex() const;
	const std::vector<Landstalker::Entity>& GetEntities() const;
	...
	const std::vector<Landstalker::WarpList::Warp>& GetWarps() const;
	void SelectWarp(int selection);
	int GetTotalWarps() const;
	bool IsWarpSelected() const;
	const Landstalker::WarpList::Warp& GetSelectedWarp() const;
	...
	const Landstalker::TileSwap& GetSelectedTileSwap() const;
	int GetSelectedTileSwapIndex() const;
	void UpdateSwaps();

	void SelectDoor(int selection);
	bool IsDoorSelected() const;
	const Landstalker::Door& GetSelectedDoor() const;
...
private:
	struct SpriteQ
	{
		...
		bool selected;
		bool background;
		std::shared_ptr<Landstalker::SpriteFrame> frame;
	...
	std::vector<std::shared_ptr<Landstalker::Palette>> PreparePalettes(uint16_t roomnum);
	...
	void DrawWarp(wxGraphicsContext& gc, int index, std::shared_ptr<Landstalker::Tilemap3D> map, int tile_width, int tile_height, bool adjust_z = false);
	void AddRoomLink(wxGraphicsContext* gc, const std::wstring& label, uint16_t room, int x, int y);
	std::unique_ptr<wxBitmap> DrawHeightmapVisualisation(std::shared_ptr<Landstalker::Tilemap3D> map, uint8_t opacity);
	...
	std::pair<int, int> GetScreenPosition(const std::pair<int, int>& iso_pos, uint16_t roomnum, Landstalker::Tilemap3D::Layer layer) const;
	void ForceRepaint();
	void ForceRedraw();
	...
	std::vector<Landstalker::TileSwap> GetPreviewSwaps();
	std::vector<Landstalker::Door> GetPreviewDoors();
	...
	uint16_t m_roomnum;
	std::vector<std::shared_ptr<Landstalker::Palette>> m_rpalette;
	...
	std::map<Layer, std::shared_ptr<ImageBufferWx>> m_layer_bufs;
	...
	std::vector<Landstalker::Door> m_doors;
