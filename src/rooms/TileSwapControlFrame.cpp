...
void TileSwapControlFrame::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
	m_gd = gd;
}
...
void TileSwapControlFrame::SetSwaps(const std::vector<Landstalker::TileSwap>& swaps, const std::vector<Landstalker::Door>& doors, uint16_t roomnum)
{
	m_swaps = swaps;
	m_doors = doors;
...
std::wstring TileSwapControlFrame::MakeLabel(int index) const
{
	if (m_selected == ID::TILESWAP)
	{
		...
		{
			return Landstalker::StrWPrintf(L"%02d: <Inactive>", index + 1);
		}
		if (m_mode == RoomEdit::Mode::BACKGROUND || m_mode == RoomEdit::Mode::FOREGROUND)
		{
			...
			return Landstalker::StrWPrintf(L"%02d: M%01d (%02d, %02d) -> (%02d, %02d) %02dx%02d", m_swaps[index].trigger,
		...
		{
			return Landstalker::StrWPrintf(L"%02d: H (%02d, %02d) -> (%02d, %02d) %02dx%02d", m_swaps[index].trigger,
	...
	{
		const auto& door_sz_name = Landstalker::Door::SIZE_NAMES.at(m_doors[index].size);
		return Landstalker::StrWPrintf(L"%02d: (%02d, %02d) %ls", index + 1, m_doors[index].x, m_doors[index].y,
