...
TileSwapDataViewModel::TileSwapDataViewModel(uint16_t roomnum, std::shared_ptr<Landstalker::GameData> gd)
	: BaseDataViewModel(),
	  m_roomnum(roomnum),
	  m_gd(gd)
{
...
bool TileSwapDataViewModel::SetValueByRow(const wxVariant& variant, unsigned int row, unsigned int col)
{
	...
	if (row < m_swaps.size())
	{
		...
		case 1:
			...
			break;
		case 2:
			m_swaps[row].mode = static_cast<Landstalker::TileSwap::Mode>(std::clamp<long>(variant.GetLong(), 0, 2));
			updated = true;
			break;
		case 3:
...
bool TileSwapDataViewModel::AddRow(unsigned int row)
{
	if (row <= m_swaps.size() && m_swaps.size() < 0x20)
	{
		...
		auto it = m_swaps.insert(m_swaps.begin() + row, Landstalker::TileSwap());
