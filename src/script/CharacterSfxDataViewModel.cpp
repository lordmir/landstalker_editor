...
CharacterSfxDataViewModel::CharacterSfxDataViewModel(std::shared_ptr<Landstalker::GameData> gd)
	: BaseDataViewModel(),
	  m_gd(gd)
{
	Initialise();
...
void CharacterSfxDataViewModel::Initialise()
{
	...
	{
		auto name = Landstalker::Labels::Get(Landstalker::Labels::C_SOUNDS, i);
		m_choices.push_back(name.value_or(Landstalker::StrWPrintf("<%02X> ???", i)));
	}
	Reset(GetRowCount());
}
...
void CharacterSfxDataViewModel::GetValueByRow(wxVariant& variant, unsigned int row, unsigned int col) const
{
	...
	{
	case 0:
		variant = Landstalker::StrWPrintf(L"(%02d) %ls", row, m_gd->GetStringData()->GetSpecialCharName(row).c_str());
		break;
	case 1:
