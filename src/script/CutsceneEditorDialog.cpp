...
CutsceneEditorDialog::CutsceneEditorDialog(wxWindow* parent, std::shared_ptr<Landstalker::GameData> gd,
	int cutscene_index)
	...
	m_index(cutscene_index)
{
	...
	{
		table = m_gd->GetScriptData()->GetCutsceneActions();
	}
	...
	const auto name = Landstalker::Labels::Get(Landstalker::Labels::C_CUTSCENE, m_index);
	const wxString generic = wxString::Format("Cutscene%03X", static_cast<unsigned>(m_index));
	wxString title = wxString::Format("Cutscene $%03X - ", static_cast<unsigned>(m_index));
	title += (name && !name->empty()) ? wxString(*name) : generic;
	SetTitle(title);

	auto* sizer = new wxBoxSizer(wxVERTICAL);
	// Editable name (C_CUTSCENE): custom value shown, generic seed as the placeholder.
	...
	Bind(wxEVT_BUTTON, &CutsceneEditorDialog::OnOk, this, wxID_OK);
}

void CutsceneEditorDialog::OnOk(wxCommandEvent& evt)
{
	std::shared_ptr<Landstalker::AsmFunctionTable> table;
	if (m_gd && m_gd->GetScriptData())
	{
	...
	if (m_name)
	{
		...
		const auto current = Landstalker::Labels::Get(Landstalker::Labels::C_CUTSCENE, m_index);
		if (!text.empty() && (!current || *current != text.ToStdWstring()))
		{
			Landstalker::Labels::Update(Landstalker::Labels::C_CUTSCENE, m_index, text.ToStdWstring());
		}
	}
