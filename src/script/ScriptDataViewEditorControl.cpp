#include <script/ScriptDataViewEditorControl.h>
...
ScriptDataViewEditorControl::ScriptDataViewEditorControl(wxWindow* parent, const wxRect& rect, uint16_t value, std::shared_ptr<const Landstalker::GameData> gd)
	...
	m_gd(gd)
{
	...
	m_string_select = new wxSpinCtrl(m_panels[EditorMode::STRING], wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS | wxWANTS_CHARS,
		m_gd->GetScriptData()->GetStringStart(), m_gd->GetStringData()->GetStringCount(Landstalker::StringData::Type::MAIN) - 1);
	...
	std::transform(Landstalker::ScriptPlayBgmEntry::BGMS.cbegin(), Landstalker::ScriptPlayBgmEntry::BGMS.cend(), std::back_inserter(bgms), [](const auto& bgm) {
		return *Landstalker::Labels::Get(Landstalker::Labels::C_SOUNDS, bgm);
	});
...
{
	m_entry = Landstalker::ScriptTableEntry::FromBytes(value);
	m_type = m_entry->GetType();
	if (m_type >= Landstalker::ScriptTableEntryType::STRING && m_type <= Landstalker::ScriptTableEntryType::PLAY_CUTSCENE)
	{
	...
	case Landstalker::ScriptTableEntryType::STRING:
		{
			const auto& string_entry = dynamic_cast<const Landstalker::ScriptStringEntry&>(*m_entry);
	...
	case Landstalker::ScriptTableEntryType::PLAY_CUTSCENE:
		{
			const auto& cutscene_entry = dynamic_cast<const Landstalker::ScriptInitiateCutsceneEntry&>(*m_entry);
	...
	case Landstalker::ScriptTableEntryType::ITEM_LOAD:
		{
			const auto& item_entry = dynamic_cast<const Landstalker::ScriptItemLoadEntry&>(*m_entry);
	...
	case Landstalker::ScriptTableEntryType::NUMBER_LOAD:
		m_value_select->SetValue(dynamic_cast<const Landstalker::ScriptNumLoadEntry&>(*m_entry).num);
		break;
	case Landstalker::ScriptTableEntryType::GLOBAL_CHAR_LOAD:
		{
			const auto& char_entry = dynamic_cast<const Landstalker::ScriptGlobalCharLoadEntry&>(*m_entry);
	...
	case Landstalker::ScriptTableEntryType::SET_FLAG:
		{
			const auto& flag_entry = dynamic_cast<const Landstalker::ScriptSetFlagEntry&>(*m_entry);
			m_value_select->SetValue(flag_entry.flag);
			m_value_preview->SetLabelText(Landstalker::ScriptData::GetFlagDisplayName(flag_entry.flag));
		}
		break;
	case Landstalker::ScriptTableEntryType::SET_GLOBAL_SPEAKER:
		{
			const auto& char_entry = dynamic_cast<const Landstalker::ScriptSetGlobalSpeakerEntry&>(*m_entry);
	...
	case Landstalker::ScriptTableEntryType::SET_SPEAKER:
		{
			const auto& char_entry = dynamic_cast<const Landstalker::ScriptSetSpeakerEntry&>(*m_entry);
			m_value_select->SetValue(char_entry.chr);
	...
	case Landstalker::ScriptTableEntryType::PLAY_BGM:
		m_bgm_select->Select(dynamic_cast<const Landstalker::ScriptPlayBgmEntry&>(*m_entry).bgm);
		break;
...
{
	m_type = Landstalker::ScriptTableEntryType::INVALID;
	int selected = m_type_select->GetSelection();
	if (selected >= (static_cast<int>(Landstalker::ScriptTableEntryType::STRING) - 1) && selected <= (static_cast<int>(Landstalker::ScriptTableEntryType::PLAY_CUTSCENE) - 1))
	{
		m_type = static_cast<Landstalker::ScriptTableEntryType>(selected + 1);
	}
	bool clear = m_entry->GetClear();
	bool end = m_entry->GetEnd();
	uint16_t data = m_entry->GetData();
	m_entry = std::move(Landstalker::ScriptTableEntry::MakeEntry(m_type));
	m_entry->SetClear(clear);
	m_entry->SetEnd(end);
	m_entry->SetData(data);
	if (m_type == Landstalker::ScriptTableEntryType::INVALID)
	{
		ShowEditor(m_type);
	}
	else
...
void ScriptDataViewEditorControl::Update()
{
	...
	case Landstalker::ScriptTableEntryType::STRING:
		{
			auto& string_entry = dynamic_cast<Landstalker::ScriptStringEntry&>(*m_entry);
			std::size_t string_idx = m_string_select->GetValue() - m_gd->GetScriptData()->GetStringStart();
			if (string_idx < m_gd->GetStringData()->GetStringCount(Landstalker::StringData::Type::MAIN))
			{
				string_entry.string = string_idx;
			}
			if (m_string_preview->GetValue().ToStdWstring() != m_gd->GetStringData()->GetString(Landstalker::StringData::Type::MAIN, m_string_select->GetValue()))
			{
		...
		break;
	case Landstalker::ScriptTableEntryType::PLAY_CUTSCENE:
		dynamic_cast<Landstalker::ScriptInitiateCutsceneEntry&>(*m_entry).cutscene = static_cast<uint16_t>(m_value_select->GetValue());
		break;
	case Landstalker::ScriptTableEntryType::ITEM_LOAD:
		{
			auto& item_entry = dynamic_cast<Landstalker::ScriptItemLoadEntry&>(*m_entry);
		...
		break;
	case Landstalker::ScriptTableEntryType::NUMBER_LOAD:
		dynamic_cast<Landstalker::ScriptNumLoadEntry&>(*m_entry).num = m_value_select->GetValue();
		break;
	case Landstalker::ScriptTableEntryType::GLOBAL_CHAR_LOAD:
		{
			auto& char_entry = dynamic_cast<Landstalker::ScriptGlobalCharLoadEntry&>(*m_entry);
			std::size_t char_idx = m_slot_value_select->GetValue();
			char_entry.slot = m_slot_select->GetSelection();
			if (char_idx < m_gd->GetStringData()->GetStringCount(Landstalker::StringData::Type::SPECIAL_NAMES))
			{
		...
		break;
	case Landstalker::ScriptTableEntryType::SET_FLAG:
		dynamic_cast<Landstalker::ScriptSetFlagEntry&>(*m_entry).flag = m_value_select->GetValue();
		break;
	case Landstalker::ScriptTableEntryType::SET_GLOBAL_SPEAKER:
		{
			auto& char_entry = dynamic_cast<Landstalker::ScriptSetGlobalSpeakerEntry&>(*m_entry);
		...
		break;
	case Landstalker::ScriptTableEntryType::SET_SPEAKER:
		dynamic_cast<Landstalker::ScriptSetSpeakerEntry&>(*m_entry).chr = m_value_select->GetValue();
		break;
	case Landstalker::ScriptTableEntryType::PLAY_BGM:
		dynamic_cast<Landstalker::ScriptPlayBgmEntry&>(*m_entry).bgm = m_bgm_select->GetSelection();
		break;
	case Landstalker::ScriptTableEntryType::INVALID:
		m_entry = std::move(Landstalker::ScriptTableEntry::FromBytes(m_value_select->GetValue()));
		break;
	case Landstalker::ScriptTableEntryType::GIVE_ITEM:
	case Landstalker::ScriptTableEntryType::GIVE_MONEY:
	default:
...
void ScriptDataViewEditorControl::ShowEditor(const Landstalker::ScriptTableEntryType& type)
{
	Freeze();
	switch (type)
	{
