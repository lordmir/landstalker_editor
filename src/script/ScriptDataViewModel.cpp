...
bool ScriptDataViewModel::SetValueByRow(const wxVariant& variant, unsigned int row, unsigned int col)
{
	...
	auto line = Landstalker::ScriptTableEntry::MakeEntry(m_script->GetScriptLine(row).GetType());
	switch(col)
	{
	case 1:
	{
	...
	case 3:
	{
		...
		m_script->SetScriptLine(row, Landstalker::ScriptTableEntry::FromBytes(static_cast<uint16_t>(variant.GetLong())));
...
bool ScriptDataViewModel::AddRow(unsigned int row)
{
	if (row <= m_script->GetScriptLineCount())
	{
		m_script->AddScriptLineBefore(row, Landstalker::ScriptTableEntry::MakeEntry(Landstalker::ScriptTableEntryType::STRING));
		RowInserted(row);
		return true;
	}
	return false;
