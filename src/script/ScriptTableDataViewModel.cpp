#include <script/ScriptTableDataViewModel.h>
...
ScriptTableDataViewModel::ScriptTableDataViewModel(std::shared_ptr<Landstalker::GameData> gd)
	: BaseDataViewModel(),
	  m_gd(gd),
...
void ScriptTableDataViewModel::GetValueByRow(wxVariant& variant, unsigned int row, unsigned int col) const
{
	...
	case 0:
		...
		case Mode::CHARACTER:
			...
			break;
		case Mode::SHOP:
			variant = wxString(SHOP_ACTIONS.at(row));
			break;
		case Mode::ITEM:
			variant = row < ITEM_ACTIONS.size() ? wxString(ITEM_ACTIONS.at(row)) : wxString(StrPrintf("Custom Event %d", row));
			break;
		default:
			variant = wxString(std::to_string(row));
			break;
		}
		break;
	case 1:
