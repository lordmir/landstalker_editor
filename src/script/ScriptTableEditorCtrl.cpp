...
void ScriptTableEditorCtrl::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
	m_gd = gd;
...
void ScriptTableEditorCtrl::OnRClick(wxMouseEvent& evt)
{
	...
	{
		Landstalker::ScriptTable::Action action = m_model->GetAction(m_last_tooltip_item);
...
void ScriptTableEditorCtrl::HandleMouseMove(const wxPoint& mouse_pos)
{
	...
	if (hovered != m_last_tooltip_item)
	{
		...
		if (hovered != -1 && m_gd)
		{
			text = Landstalker::ScriptTable::GetActionDescription(m_model->GetAction(hovered), m_gd);
			if (text.find(L"\n") == std::string::npos)
			{
				text.clear();
			}
