...
#include <script/ScriptTableEditorCtrl.h>
...
void ScriptTableEditorFrame::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
	m_gd = gd;
	...
	if (m_gd && m_gd->GetScriptData()->HasTables())
	{
		...
		if (fd.ShowModal() != wxID_CANCEL)
		{
			...
			case ScriptTableDataViewModel::Mode::CUTSCENE:
				ofs << Landstalker::ScriptTable::TableToYaml(m_gd->GetScriptData()->GetCutsceneTable());
				break;
			case ScriptTableDataViewModel::Mode::CHARACTER:
				ofs << Landstalker::ScriptTable::TableToYaml(m_gd->GetScriptData()->GetCharTable());
				break;
			case ScriptTableDataViewModel::Mode::SHOP:
				ofs << Landstalker::ScriptTable::TableToYaml(m_gd->GetScriptData()->GetShopTable());
				break;
			case ScriptTableDataViewModel::Mode::ITEM:
				ofs << Landstalker::ScriptTable::TableToYaml(m_gd->GetScriptData()->GetItemTable());
				break;
			}
		}
	}
	...
	if (m_gd && m_gd->GetScriptData()->HasTables())
	{
		...
		if (fd.ShowModal() != wxID_CANCEL)
		{
			try
			{
				...
				{
				case ScriptTableDataViewModel::Mode::CUTSCENE:
					*m_gd->GetScriptData()->GetCutsceneTable() = Landstalker::ScriptTable::TableFromYaml(yaml.str());
					break;
				case ScriptTableDataViewModel::Mode::CHARACTER:
					*m_gd->GetScriptData()->GetCharTable() = Landstalker::ScriptTable::TableFromYaml(yaml.str());
					break;
				case ScriptTableDataViewModel::Mode::SHOP:
					*m_gd->GetScriptData()->GetShopTable() = Landstalker::ScriptTable::ShopTableFromYaml(yaml.str());
					break;
				case ScriptTableDataViewModel::Mode::ITEM:
					*m_gd->GetScriptData()->GetItemTable() = Landstalker::ScriptTable::ItemTableFromYaml(yaml.str());
					break;
				}
				FireEvent(EVT_PROPERTIES_UPDATE);
...
void ScriptTableEditorFrame::OnNewCollection()
{
	...
	{
		Landstalker::ScriptTable::Shop shop;
		std::string new_item = Landstalker::StrPrintf("Script/Script Tables/Shop Tables/ShopTable%d", m_gd->GetScriptData()->GetShopTable()->size());
	...
	{
		Landstalker::ScriptTable::Item item;
		std::string new_item = Landstalker::StrPrintf("Script/Script Tables/Item Tables/ItemTable%d", m_gd->GetScriptData()->GetItemTable()->size());
...
void ScriptTableEditorFrame::OnDeleteCollection()
{
	if (m_mode == ScriptTableDataViewModel::Mode::SHOP)
	{
		std::shared_ptr<std::vector<Landstalker::ScriptTable::Shop>> shop_table = m_gd->GetScriptData()->GetShopTable();
		if (shop_table->size() == 0)
		{
			return;
		}
		std::string item_to_remove = Landstalker::StrPrintf("Script/Script Tables/Shop Tables/ShopTable%d", shop_table->size() - 1);
		std::string item_to_select = "Script/Script Tables/Character Table";
		if (m_index < shop_table->size() - 1)
		{
			item_to_select = Landstalker::StrPrintf("Script/Script Tables/Shop Tables/ShopTable%d", m_index);
		}
		else if (m_index > 0)
		{
			item_to_select = Landstalker::StrPrintf("Script/Script Tables/Shop Tables/ShopTable%d", m_index - 1);
		}
	...
	{
		std::shared_ptr<std::vector<Landstalker::ScriptTable::Item>> item_table = m_gd->GetScriptData()->GetItemTable();
		if (item_table->size() == 0)
		{
			return;
		}
		std::string item_to_remove = Landstalker::StrPrintf("Script/Script Tables/Item Tables/ItemTable%d", item_table->size() - 1);
		std::string item_to_select = "Script/Script Tables/Character Table";
		if (m_index < item_table->size() - 1)
		{
			item_to_select = Landstalker::StrPrintf("Script/Script Tables/Item Tables/ItemTable%d", m_index);
		}
		else if (m_index > 0)
		{
			item_to_select = Landstalker::StrPrintf("Script/Script Tables/Item Tables/ItemTable%d", m_index - 1);
		}
