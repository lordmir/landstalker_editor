...
void EntityViewerCtrl::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
	m_gd = gd;
	Refresh(true);
}
...
void EntityViewerCtrl::SetActivePalette(const std::shared_ptr<Landstalker::Palette> pal)
{
	m_palette = pal;
	Refresh(true);
}
...
void EntityViewerCtrl::Open(uint8_t entity, uint8_t animation, std::shared_ptr<Landstalker::Palette> pal)
{
	m_entity_id = entity;
...
void EntityViewerCtrl::Open(std::shared_ptr<Landstalker::SpriteFrame> frame, std::shared_ptr<Landstalker::Palette> pal)
{
	m_entity_id = 0;
	m_animation = 0;
	m_frame = 0;
