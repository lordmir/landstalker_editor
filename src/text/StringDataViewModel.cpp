...
unsigned int StringDataViewModel::GetColumnCount() const
{
    ...
    else if (m_type == Landstalker::StringData::Type::END_CREDITS)
    {
        return 4;
    }
    return 2;
...
std::string StringDataViewModel::GetColumnHeader(unsigned int col) const
{
    ...
    else if (m_type == Landstalker::StringData::Type::INTRO)
    {
        switch (col)
        {
        case 1:
...
wxString StringDataViewModel::GetColumnType(unsigned int col) const
{
    ...
    else if (m_type == Landstalker::StringData::Type::INTRO && col < 6)
    {
        return "long";
    }
    else if (m_type == Landstalker::StringData::Type::END_CREDITS && col < 3)
    {
        return "long";
    }
    return "string";
...
bool StringDataViewModel::SetValueByRow(const wxVariant& variant, unsigned int row, unsigned int col)
{
    ...
    {
        Landstalker::IntroString news = m_sd->GetIntroString(row);
        switch (col)
        {
        case 1:
    ...
    {
        Landstalker::EndCreditString news = m_sd->GetEndCreditString(row);
        switch (col)
        {
        case 1:
