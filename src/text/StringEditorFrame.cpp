...
StringEditorFrame::StringEditorFrame(wxWindow* parent, ImageList* imglst)
	: EditorFrame(parent, wxID_ANY, imglst),
      m_type(Landstalker::StringData::Type::MAIN),
...
void StringEditorFrame::SetMode(Landstalker::StringData::Type type)
{
    ...
    if (type == Landstalker::StringData::Type::INTRO)
    {
    ...
    else if (type == Landstalker::StringData::Type::END_CREDITS)
    {
        m_stringView->InsertColumn(1, new wxDataViewColumn(m_model->GetColumnHeader(1),
...
void StringEditorFrame::OnMenuImport()
{
    ...
    if (m_type == Landstalker::StringData::Type::INTRO || m_type == Landstalker::StringData::Type::END_CREDITS)
    {
...
void StringEditorFrame::OnAppend()
{
    ...
    UpdateUI();
    m_stringView->SetCurrentItem(wxDataViewItem(reinterpret_cast<void*>(static_cast<std::intptr_t>(m_model->GetRowCount()))));
...
void StringEditorFrame::OnDelete()
{
    if (m_stringView->HasSelection())
    {
        ...
        {
            m_stringView->Select(wxDataViewItem(reinterpret_cast<void*>(static_cast<std::intptr_t>(m_model->GetRowCount()))));
            m_stringView->EnsureVisible(m_stringView->GetSelection());
        }
    }
    UpdateUI();
...
{
    return m_type != Landstalker::StringData::Type::DEFAULT_NAME && m_type != Landstalker::StringData::Type::SYSTEM;
}
...
void StringEditorFrame::SetGameData(std::shared_ptr<Landstalker::GameData> gd)
{
	m_gd = gd;
    m_type = Landstalker::StringData::Type::MAIN;
    Update();
...
bool StringEditorFrame::ExportStrings(const std::filesystem::path& filename, Landstalker::StringData::Type mode)
{
    bool retval = true;
    ...
    switch (mode)
    {
    case Landstalker::StringData::Type::INTRO:
        ...
        break;
    case Landstalker::StringData::Type::END_CREDITS:
...
bool StringEditorFrame::ImportStrings(const std::filesystem::path& filename, Landstalker::StringData::Type mode)
{
    bool retval = true;
    std::ifstream fs;
    std::ostringstream errorss;
    std::vector<Landstalker::LSString> lines;
    std::wstring_convert<std::codecvt_utf8<Landstalker::LSString::StringType::value_type>> utf8_conv;
    auto sd = m_gd->GetStringData();
    ...
    else
    {
        ...
        std::wstring wline;
        int line_count = 0;
        if (mode == Landstalker::StringData::Type::INTRO || mode == Landstalker::StringData::Type::END_CREDITS)
        {
        ...
        while (fs.eof() == false)
        {
            ...
            if (line.empty() == false)
            {
                ...
                {
                case Landstalker::StringData::Type::INTRO:
                    sd->SetIntroString(line_count++, Landstalker::IntroString(wline));
                    break;
                case Landstalker::StringData::Type::END_CREDITS:
                    sd->SetEndCreditString(line_count++, Landstalker::EndCreditString(wline));
                    break;
                default:
