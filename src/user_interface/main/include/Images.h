...
#include <landstalker/main/include/Blockmap2D.h>
#include <landstalker/main/include/PaletteO.h>
...
namespace Images
{
	struct Image
	{
		...
		std::shared_ptr<PaletteO> palette;
	};
	...
	static std::map<std::string, Image> GetImages(const Rom& m_rom)
	{
		...
		for (const auto& elem : IMAGES)
		{
			...
			auto pal = std::make_shared<PaletteO>(PaletteO(m_rom, d.palette_idx, d.palette_type));
			ret[name] = { ts, map, pal };
		}
		return ret;
	}
