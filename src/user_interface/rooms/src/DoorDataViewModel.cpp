#include <user_interface/rooms/include/DoorDataViewModel.h>
...
wxArrayString DoorDataViewModel::GetColumnChoices(unsigned int col) const
{
	...
	if (col == 3)
	{
		for (const auto& s : Door::SIZE_NAMES)
		{
...
bool DoorDataViewModel::SetValueByRow(const wxVariant& variant, unsigned int row, unsigned int col)
{
	...
	if (row < m_doors.size())
	{
		...
		case 2:
			...
			break;
		case 3:
			m_doors[row].size = static_cast<Door::Size>(std::clamp<long>(variant.GetLong(), 0, 7));
			updated = true;
			break;
		default:
			break;
...
bool DoorDataViewModel::AddRow(unsigned int row)
{
	if (row <= m_doors.size())
	{
		m_doors.insert(m_doors.begin() + row, Door());
		Reset(GetRowCount());
		return true;
	}
	return false;
