#include <user_interface/script/include/CharacterSfxCtrl.h>
...
void CharacterSfxEditorCtrl::SetGameData(std::shared_ptr<GameData> gd)
{
	m_gd = gd;
...
void CharacterSfxEditorCtrl::Open(int chr)
{
	if (m_model)
	{
		...
		if (chr >= 0)
		{
			m_dvc_ctrl->EnsureVisible(wxDataViewItem(reinterpret_cast<void*>(chr)));
		}
	}
}
